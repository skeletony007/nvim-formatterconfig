local configs = require("formatterconfig.configs")
...
---@field inconfig? boolean should display in healthcheck (`:checkhealth formatterconfig`)
local aliases = {
    ["example"] = {
        to = "new_example",
        version = "0.2.1",
...
function mt:__index(k)
    if configs[k] == nil then
        ...
        if alias then
            vim.deprecate(k, alias.to, alias.version, "nvim-formatterconfig", false)
            alias.inconfig = true
            k = alias.to
        end

        local success, config = pcall(require, "formatterconfig.configs." .. k)
        if success then
            configs[k] = config
        else
            vim.notify(string.format('[formatterconfig] config "%s" not found.', k), vim.log.levels.WARN)
